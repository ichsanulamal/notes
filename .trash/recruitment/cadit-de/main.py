...
def clean(df):
    ...
    df['year_start'] = df['year'].apply(lambda x: x[0] if isinstance(x, list) and len(x) > 0 else None)
    df['year_end'] = df['year'].apply(lambda x: x[1] if isinstance(x, list) and len(x) > 1 else None)

...
def csv2pg(conn_id = 'pg_local', target_table='public.movies', csv_path='/home/al/projects/detest_linkaja/.temp/IMDB-Movie-Data.csv'):
    sql = f'COPY {target_table} FROM stdin WITH CSV HEADER'
    
    pg_hook = PostgresHook(postgres_conn_id=conn_id)
    pg_hook.copy_expert(sql, csv_path)

def pg2json(conn_id = 'pg_local', source_table='public.movies_top_animation', target_path = '/home/al/projects/detest_linkaja/.temp/output.json'):
...
def json2bq(target_table='ichsanul-dev.dev.movies_top_animation', json_path='//home/al/projects/detest_linkaja/.temp/output.json'):
    # Connect to BigQuery
...
with DAG(
    ...
    # pg
    delete_pg_task = SQLExecuteQueryOperator(task_id = 'delete_pg_task', conn_id = 'pg_local', sql='DELETE FROM public.movies;')
